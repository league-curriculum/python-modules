...
def draw():
    ...
    if lower_pipe.x < -lower_pipe.width and upper_pipe.x < -upper_pipe.width:
        reset_pipes(lower_pipe, upper_pipe) 
    ...
    collision = intersects_pipes(bird, lower_pipe, upper_pipe)
    
    if collision or bird.y < 0 or bird.y + bird.height > height:
        noLoop()
